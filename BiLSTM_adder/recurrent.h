...
public:
	...
	virtual void accum_forward(double*& next_port) {
		...
		km_1d::add(next_port, hidden, output_size);
	}
...
public:
	...
	virtual void accum_forward(double*& next_port) {
		...
		km_1d::add(next_port, short_term, output_size);
	}
...
public:
	StackBundle* fnn=nullptr;
	StackBundle* bnn = nullptr;
	StackBundle* merger = nullptr;
	double** bidir_out = nullptr;

	int data_len = 0;
	int n_storage=0;
	BidirectionalModel(StackBundle* _frnn, StackBundle* _brnn, StackBundle* _mnn) {
		fnn = _frnn;
		bnn = _brnn;
		merger = _mnn;
	}
	void FPTT(double** const& input, double**& output, int sq_len) {
		...
		for (int t = 0; t < data_len; t++) {
			fnn->charge(input[t]);
			fnn->overwrite_forward(bidir_out[t]);
			fnn->storeAt(t);
		}
		bnn->init_fptt();
		for (int t = data_len-1; t >= 0; t--) {
			bnn->charge(input[t]);
			bnn->accum_forward(bidir_out[t]);
			bnn->storeAt(t);
		}
		for (int t = 0; t < data_len; t++) {
			merger->charge(bidir_out[t]);
			merger->overwrite_forward(output[t]);
			merger->storeAt(t);
		}
	}
	...
	virtual void allocMemory(int num_of_port) {
		...
		fnn->allocMemory(n_storage);
		bnn->allocMemory(n_storage);
		merger->allocMemory(n_storage);
		bidir_out = km_2d::alloc(n_storage, fnn->output_size);
	}
	...
	void BPTT(double** const& out_grad) {
		for (int t = 0; t < data_len; t++) {
			merger->pointAt(t);
			merger->overwrite_backward(out_grad[t]);
			km_1d::copy(bidir_out[t], merger->_in_, merger->input_size);
		}
		bnn->init_bptt();
		for (int t = data_len - 1; t >= 0; t--) {
			bnn->pointAt(t);
			bnn->overwrite_backward(bidir_out[t]);
		}
		fnn->init_bptt();
		for (int t = 0; t < data_len; t++ ) {
			fnn->pointAt(t);
			fnn->overwrite_backward(bidir_out[t]);
		}
	}
};